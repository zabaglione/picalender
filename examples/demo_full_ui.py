#!/usr/bin/env python3
"""
フルUI統合デモ

時計、日付、カレンダーを全て表示するサンプル実装
"""

import sys
import os
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

import pygame
from src.ui.clock_renderer import ClockRenderer
from src.ui.date_renderer import DateRenderer
from src.ui.calendar_renderer import CalendarRenderer
from src.assets.asset_manager import AssetManager
from src.core.config_manager import ConfigManager


def main():
    """メイン処理"""
    # pygame初期化
    pygame.init()
    
    # 設定とアセット管理の初期化
    config = ConfigManager()
    asset_manager = AssetManager(base_path="assets")
    
    # 画面の作成
    screen_width = config.get('screen.width', 1024)
    screen_height = config.get('screen.height', 600)
    screen = pygame.display.set_mode((screen_width, screen_height))
    pygame.display.set_caption("Full UI Demo - Clock, Date & Calendar")
    
    # レンダラーの作成
    clock_renderer = ClockRenderer(
        asset_manager=asset_manager,
        config=config
    )
    
    date_renderer = DateRenderer(
        asset_manager=asset_manager,
        config=config
    )
    
    calendar_renderer = CalendarRenderer(
        asset_manager=asset_manager,
        config=config
    )
    
    # FPS管理
    clock = pygame.time.Clock()
    target_fps = 30
    
    # 背景色（深い青灰色）
    bg_color = (30, 35, 45)
    
    # メインループ
    running = True
    while running:
        dt = clock.tick(target_fps) / 1000.0  # ミリ秒を秒に変換
        
        # イベント処理
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                running = False
            elif event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:
                    running = False
                elif event.key == pygame.K_SPACE:
                    # スペースキーで全表示/非表示切り替え
                    visible = not clock_renderer.visible
                    clock_renderer.set_visible(visible)
                    date_renderer.set_visible(visible)
                    calendar_renderer.set_visible(visible)
                elif event.key == pygame.K_c:
                    # Cキーでカレンダーの表示/非表示切り替え
                    calendar_renderer.set_visible(not calendar_renderer.visible)
                elif event.key == pygame.K_j:
                    # Jキーで日本語曜日
                    date_renderer.set_weekday_language('jp')
                    calendar_renderer.set_weekday_language('jp')
                elif event.key == pygame.K_e:
                    # Eキーで英語曜日
                    date_renderer.set_weekday_language('en')
                    calendar_renderer.set_weekday_language('en')
                elif event.key == pygame.K_1:
                    # 1キーで時計の色を変更（赤）
                    clock_renderer.set_color((255, 100, 100))
                elif event.key == pygame.K_2:
                    # 2キーで時計の色を変更（緑）
                    clock_renderer.set_color((100, 255, 100))
                elif event.key == pygame.K_3:
                    # 3キーで時計の色を変更（青）
                    clock_renderer.set_color((100, 150, 255))
                elif event.key == pygame.K_0:
                    # 0キーで時計の色をリセット
                    clock_renderer.set_color((255, 255, 255))
        
        # 更新処理
        clock_renderer.update(dt)
        date_renderer.update(dt)
        calendar_renderer.update(dt)
        
        # 描画処理
        need_redraw = (clock_renderer.is_dirty() or 
                      date_renderer.is_dirty() or 
                      calendar_renderer.is_dirty())
        
        if need_redraw:
            # 全画面クリア
            screen.fill(bg_color)
            
            # 背景パターンを描画（装飾）
            for y in range(0, screen_height, 50):
                pygame.draw.line(screen, (35, 40, 50), (0, y), (screen_width, y), 1)
            for x in range(0, screen_width, 50):
                pygame.draw.line(screen, (35, 40, 50), (x, 0), (x, screen_height), 1)
            
            # UI要素を描画
            clock_renderer.render(screen)
            date_renderer.render(screen)
            calendar_renderer.render(screen)
            
            # 操作説明を表示
            font = pygame.font.Font(None, 18)
            instructions = [
                "Controls:",
                "  ESC: Exit",
                "  SPACE: Toggle all",
                "  C: Toggle calendar",
                "  J/E: JP/EN weekday",
                "  1/2/3/0: Clock colors"
            ]
            y = 10
            for text in instructions:
                rendered = font.render(text, True, (80, 80, 100))
                screen.blit(rendered, (10, y))
                y += 20
            
            # ステータス表示
            status_texts = [
                f"Lang: {date_renderer.weekday_lang}",
                f"FPS: {clock.get_fps():.1f}"
            ]
            y = screen_height - 50
            for text in status_texts:
                rendered = font.render(text, True, (80, 80, 100))
                screen.blit(rendered, (10, y))
                y += 20
            
            # 画面更新
            pygame.display.flip()
        
        # FPS表示
        pygame.display.set_caption(f"Full UI Demo - FPS: {clock.get_fps():.1f}")
    
    # 終了処理
    pygame.quit()


if __name__ == "__main__":
    main()